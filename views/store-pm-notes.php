...
<div class="pm-note-actions"><form method="post" action="<?= e(url('store',['id'=>$store['id']])) ?>" class="pm-note-report-form"><?= csrf() ?><input type="hidden" name="action" value="pm-note-report"><input type="hidden" name="note_id" value="<?= e($pmNote['id']) ?>"><label class="checkbox-label"><input type="checkbox" name="include_in_report" value="1" <?= $pmNote['include_in_report']?'checked':'' ?>>Include in PDF report</label><span class="pm-note-feedback field-help" role="status" aria-live="polite"></span></form>
<form method="post" action="<?= e(url('store',['id'=>$store['id']])) ?>" class="pm-note-delete-form"><?= csrf() ?><input type="hidden" name="action" value="delete-pm-note"><input type="hidden" name="note_id" value="<?= e($pmNote['id']) ?>"><button type="submit" class="button pm-note-delete" disabled>Delete note</button></form></div>
